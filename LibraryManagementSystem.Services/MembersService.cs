...
public sealed class MembersService
{
    ...
    {
        Members? Members = _dbContext.Members.FirstOrDefault(m => m.MemberId == MemberId);
        if (Members is null)
        {
            return null;
        }
    ...
    public MembersDto? CreateMemberRequest(CreateMemberRequest request)
    {
        try
        {
            Members Members = new() { MemberName = request.MemberName, MemberType = request.MemberType };
            _dbContext.Members.Add(Members);
            _dbContext.SaveChanges();

            MembersDto MembersDto = new(
                Members.MemberId,
                Members.MemberName,
                Members.MemberType);
            return MembersDto;
    ...
    public MembersDto? UpdateMemberRequest(CreateMemberRequest request, int MemberId)
    {
        try
        {
            Members? Members = _dbContext.Members.Find(MemberId);
            if (Members is null)
            {
